...
async def generate_detailed_horoscope(name: str, query: str, sign: str):
    ...
    prompt = (
        ...
        "ВАЖНО: Если в его сообщении есть конкретная тема (друзья, встречи, любовь, работа, деньги, казино, Deadlock и т.п.) — отвечай ТОЛЬКО про эту тему, коротко, 1-2 предложения. "
        "НЕ пересказывай весь гороскоп и НЕ добавляй другие сферы жизни. "
        "Развёрнутый ответ (до 4 предложений) давай ТОЛЬКО если он явно просит «подробнее», «полный гороскоп», «развёрнутый» или не спрашивает ни о чём конкретном."
    )
