...
async def generate_detailed_horoscope(name: str, query: str, sign: str):
    ...
    prompt = (
        ...
        "ВАЖНО: Если в его сообщении есть конкретная тема (друзья, встречи, любовь, работа, деньги, казино, Deadlock и т.п.) — отвечай ТОЛЬКО про эту тему. "
        "НЕ пересказывай весь гороскоп и НЕ добавляй другие сферы жизни. "
        "Полный развёрнутый гороскоп давай ТОЛЬКО если он явно просит «подробнее», «полный гороскоп», «развёрнутый» или не спрашивает ни о чём конкретном. "
        "2-4 предложения."
    )
